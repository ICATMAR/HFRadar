...
HFANTENNAS = [
  ...
  loadDataFromRepository = function(timestamp, fileTypes){
    ...
    if (fileTypes.includes('ruv')){
      for (let i = 0; i < HFANTENNAS.length; i++){
        urls.push(baseURL + 'L2/' + HFANTENNAS[i] + '/' + year + '/' + month + '/RDLm_' + HFANTENNAS[i] + '_' + year + '_' + month + '_' + day + '_' + hour + '00_l2b.ruv');
      }
    }
    // Waves and wind
    if (fileTypes.includes('wls')){
      for (let i = 0; i < HFANTENNAS.length; i++){
        urls.push(wavesBaseURL + HFANTENNAS[i] + '/WVLM_' + HFANTENNAS[i] + '_' + year + '_' + month + '_01_0000.wls');
      }
    }
